...
def run_complete_simulation(pathology_description: str):
    ...
    print(
        f"   - Antécédents : {', '.join(patient.antecedents) if patient.antecedents else 'Aucun'}"
    )
    ...
    if patient.constantes:
        ...
        print(f"   - SpO2 : {patient.constantes.spo2}%")
        print(
            f"   - TA : {patient.constantes.ta_systolique}/{patient.constantes.ta_diastolique} mmHg"
        )
        print(f"   - Température : {patient.constantes.temperature}°C")
    ...
    return {"patient": patient, "conversation": conversation}
