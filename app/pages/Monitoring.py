...
if latency_stats:
    ...
    with col2:
        ...
        for lat in tracker.latencies:
            latency_data.append({
                "Composant": lat["component"],
                "Durée (s)": lat["duration"]
            })
...
if pred_stats['total'] > 0:
    ...
    with col1:
        ...
        severity_df = pd.DataFrame([
            {"Niveau": k, "Nombre": v}
            for k, v in pred_stats['by_severity'].items()
        ])
...
# Footer
st.divider()
st.caption("💡 Note: Les coûts sont estimés selon les tarifs Mistral officiels, même avec clé gratuite.")