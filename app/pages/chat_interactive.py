...
if "chatbot" not in st.session_state:
    ...
    with st.spinner("🔄 Chargement RAG..."):
        try:
            ...
            print(f"⚠️ RAG non chargé: {e}")
            st.session_state.predictor = MLTriagePredictor()
    
    st.session_state.chatbot = TriageChatbotAPI()
...
with col2:
    ...
    if st.session_state.prediction:
        ...
        st.info("💡 Complétez la conversation (5/5 constantes) puis cliquez 'Obtenir prédiction ML'")
...
st.divider()
st.caption("🤖 ML (Random Forest) + 📚 RAG (Documents médicaux) + 🔮 Mistral API | ⚠️ Outil d'aide - ne remplace pas avis médical")